...
- origin_analysis: 원산지 분석 (FTA·원산지증명·우회수입 가능성)
...
[에이전트 실행 순서 원칙]
company → db → rag류 → audit_search → ml → network → web → declaration_verify → hs_verify → customs_value → ontology → origin_analysis → abnormal_trade → proceeds_tracking → route_analysis → patent → law → report → validate
...
_DATA_SOURCES = {
    ...
    "rag_customs":     "관세e음 RAG — agent_rag.py의 rag_customs 컬렉션 검색",
    "rag_trade":       "통관정보 RAG — agent_rag.py의 rag_trade 컬렉션 검색",
    "rag_audit":       "심사정보 RAG — agent_rag.py의 rag_audit 컬렉션 검색",
    "rag_investigation": "조사정보 RAG — agent_rag.py의 rag_investigation 컬렉션 검색",
    "rag_global":      "국제정보 RAG — agent_rag.py의 rag_global 컬렉션 검색",
}
...
_AGENTS = {
    ...
    "origin_analysis":    "원산지 분석 — FTA·원산지증명·우회수입 가능성 시뮬레이션",
...
def _strip_internal_tool_directives(text: str) -> str:
    ...
    replacements = [
        ...
        ("rag_trade와 rag_audit 데이터소스를 활용하고, law 에이전트를 사용하여 ", ""),
        ("rag_trade와 rag_audit 데이터소스를 활용하고, law 에이전트를 사용해줘", ""),
    ...
    cleaned = re.sub(r"\b(?:rag_[a-z_]+|db_cdw|hs_verify|customs_value|law|ml|ontology|origin_analysis|abnormal_trade|proceeds_tracking|route_analysis|network|web|patent|rag_create|summary|report|validate)\b(?:와|과|,|\s)*(?:\b(?:rag_[a-z_]+|db_cdw|hs_verify|customs_value|law|ml|ontology|origin_analysis|abnormal_trade|proceeds_tracking|route_analysis|network|web|patent|rag_create|summary|report|validate)\b(?:와|과|,|\s)*)*(?:데이터소스|RAG|에이전트|Agent)(?:를|을)?\s*(?:활용하고|활용하여|사용하고|사용하여|사용해줘|참조하여)?", "", cleaned, flags=re.IGNORECASE)
