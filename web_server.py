...
[대상 특정 여부 — 매우 중요]
- 아래 에이전트는 '특정 기업/개인 대상'이 있어야만 의미가 있다(대상특정 필수):
  company, db, ml, network, declaration_verify, hs_verify, customs_value,
  origin_analysis, abnormal_trade, proceeds_tracking, route_analysis
- 프롬프트에 회사ID(C-XXXX)·회사명·특정 대상이 없으면 위 에이전트는 선택하지 마라.
  대신 대상이 필요 없는 에이전트(rag류·audit_search·web·patent·law·ontology)만 선택한다.
- 특정 대상이 없고 지식/사례/법령 검색이면 관련 RAG·law·web 만 선택한다.
...
# 특정 기업/개인 대상이 있어야만 의미 있는 에이전트 — 대상 미특정 시 라우팅에서 제외
_TARGET_REQUIRED_AGENTS = {
    "company", "db", "ml", "network", "declaration_verify", "hs_verify",
