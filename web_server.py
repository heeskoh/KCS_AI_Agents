...
_INTERNAL_TOOL_WORDS = (
    ...
    "과세가격평가", "품목분류검증", "hs_verify", "customs_value",
)
...
def _strip_internal_tool_directives(text: str) -> str:
    ...
    cleaned = re.sub(r"\b(?:rag_[a-z_]+|db_cdw|hs_verify|customs_value|law|ml)\b(?:와|과|,|\s)*(?:\b(?:rag_[a-z_]+|db_cdw|hs_verify|customs_value|law|ml)\b(?:와|과|,|\s)*)*(?:데이터소스|RAG|에이전트|Agent)(?:를|을)?\s*(?:활용하고|활용하여|사용하고|사용하여|사용해줘|참조하여)?", "", cleaned, flags=re.IGNORECASE)
