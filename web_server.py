    ...
    with duckdb.connect(str(DB_PATH), read_only=True) as conn:
        ...
        unit_sql = """
            SELECT strftime(d.import_date, '%Y-%m') AS ym,
                   ...
                   count(*) AS n,
                   round(sum(d.declared_value), 0) AS amount
...
[사용 가능한 에이전트]
- ocr: OCR/문서인식 — 첨부 파일의 내용 인식·추출(파일을 읽어야 하는 모든 작업의 선행 단계)
- summary: 파일 요약 — 첨부 문서·선행 결과를 조사관용 핵심 요약으로 정리
- rag_create: 업무특화RAG 생성 — 첨부 문서를 임베딩해 검색 가능한 지식기반(RAG/검색DB) 구성
- company: 기업 기본정보 조회 (회사명·업종·수입실적)
- db: CDW 자연어조회 (DuckDB 내 수입신고·기업·위험정보만 사용)
- rag_customs: 관세정보 RAG
- rag_trade: 무역정보 RAG
- rag_audit: 심사정보 RAG
- rag_investigation: 조사정보 RAG
- rag_global: 국제협력 RAG
- clearance_report: 통관보고서 생성 (신고번호 1건 + 현장 사진 → 신고내용 조회·증빙 첨부·통관의견)
- rag_item: 품목정보 RAG (HSK 품목별 품명·규격·성분·수입요건·신고 유의사항 — 특정 물품의 신고요령·품목분류 근거)
...
- address_check: 주소확인 (도로명/지번 주소 → 도로명주소·건축물대장 주용도로 주거/사업장/혼합/판단불가 판정)

[대상 특정 여부 — 매우 중요]
- 아래 에이전트는 '특정 기업/개인 한 대상'을 분석·평가하므로 대상이 있어야만 의미가 있다(대상특정 필수):
  company, ml, declaration_verify, hs_verify, customs_value,
  origin_analysis, abnormal_trade, proceeds_tracking, route_analysis, network
- 프롬프트에 회사ID(C-XXXX)·회사명 등 특정 대상이 없으면 위 에이전트는 선택하지 마라.
- 단, db(CDW 자연어조회)는 예외다: 조건에 맞는 기업을 '찾거나' 집계·목록화하는 검색 도구다.
  "품목 오류 의심 기업 10곳 조회", "저가신고 상위 업체 목록", "특정 기업의 해외거래처 주소" 처럼
  조건 검색·필터·목록 요청이면 대상 미특정이라도 반드시 db 를 선택한다(내부 DB 쿼리).
- 지식/사례/법령 검색이면 관련 RAG·law·web 을 선택한다.

[판단 기준]
1. 특정 기업(회사ID·회사명) 분석/조사/위험평가 → mode=agents, company+db 로 시작 + 필요한 검증 에이전트
2. 법령·판례·통관절차 문의 → mode=agents, rag_customs/law (대상특정 에이전트 금지)
3. HS코드·품목분류·특정 물품의 신고요령/수입요건 문의(대상 기업 없음) → mode=agents, rag_item(+rag_customs, +web)
   — 특정 품목(예: "전동 장난감 자동차", "니코틴 전자담배")의 신고 품명·규격·수입요건은 rag_item을 우선 선택. hs_verify는 대상 있을 때만
4. 관세·무역 관련 일반 질문(DB 조회 불필요) → mode=agents, RAG 에이전트만 선택
5. 주소(도로명/지번)를 주고 주거지·사업장 여부·건물용도 확인 요청 → mode=agents, agents=["address_check"]
   (기업 대상 불필요 — '주소' 자체가 입력이다. 회사ID/회사명이 없어도 반드시 선택한다)
6. 관세/무역/통관과 전혀 무관한 일반 지식 질문 → mode=llm_direct, llm_answer에 바로 답변
7. 첨부 파일에 대한 작업 → 요청 의도에 맞는 파일 서비스를 선택한다(모두 ocr 로 내용을 먼저 인식)
   - 내용 인식·판독·추출·"읽어줘"        → agents=["ocr"]
   - 요약·핵심 정리                      → agents=["ocr", "summary"]
   - RAG·검색DB·지식기반·지식베이스 구성 → agents=["ocr", "rag_create"]

[보고서 생성 금지]
- 조회/검색/분석 요청에 report·validate 는 선택하지 마라. 검색 결과를 보고서 형태로
  만드는 기능은 사용하지 않는다(각 에이전트의 결과를 그대로 제시). 보고서 작성은
  별도의 '분석 보고서 및 검증' 워크벤치에서만 수행한다.

[에이전트 실행 순서 원칙]
company → db → rag류 → audit_search → ml → network → web → declaration_verify → hs_verify → customs_value → ontology → origin_analysis → abnormal_trade → proceeds_tracking → route_analysis → patent → law → mail_share
...
# 특정 기업/개인 '한 대상'을 분석·평가하는 에이전트 — 대상 미특정 시 라우팅에서 제외.
# 주의: db(CDW 자연어조회)는 조건 검색·집계(예: "품목 오류 의심 기업 10곳 찾기")를 위한
#       전사 검색 도구이므로 대상특정이 필요 없다 → 목록에서 제외한다.
_TARGET_REQUIRED_AGENTS = {
    "company", "ml", "declaration_verify", "hs_verify",
    "customs_value", "origin_analysis", "abnormal_trade", "proceeds_tracking",
    "route_analysis",
}
# 검색/조회 결과를 보고서 형태로 만드는 에이전트 — 자동 라우팅에서 제외(보고서 워크벤치 전용)
_REPORT_AGENTS = {"report", "validate", "report_standard", "result_synthesis"}

# 입력값 2차 검토용 — 도로명(…로/길/대로 12) 또는 지번(…동 123-4) 형태의 한국 주소 신호
_ADDRESS_SIGNAL = re.compile(
    r"[가-힣]+(?:특별시|광역시|특별자치시|특별자치도|도|시)?\s*[가-힣]+(?:시|군|구)\s*"
    r"[가-힣0-9·\s]*?(?:로|길|대로)\s*\d+(?:-\d+)?"
    r"|[가-힣]+(?:시|군|구)\s*[가-힣]+(?:동|읍|면)\s*\d+(?:-\d+)?"
)


def _recheck_by_input_signals(prompt: str, result: dict) -> None:
    """의도분석 2차 검토 — LLM 판단을 실제 입력값 신호로 다시 확인·보정한다.
    (A) 주소(도로명/지번)가 있으면 주소확인을 반드시 선정(LLM이 놓쳐도).
    (B) 대상기업이 특정되면 관련 서비스 기본세트(기업정보·CDW)를 먼저 선정한다."""
    agents = list(result.get("agents") or [])
    notes: list[str] = []
    # (A) 주소 입력 신호 → 주소확인 강제 포함
    if _ADDRESS_SIGNAL.search(prompt or "") and "address_check" not in agents:
        agents = ["address_check"] + agents
        result["mode"] = "agents"
        notes.append("입력값 재검토: 주소 감지 → 주소확인 선정")
    # (B) 대상기업 특정 → 관련 서비스 기본세트(기업정보·CDW)가 하나도 없으면 우선 선정
    if result.get("company_id") and not ({"company", "db"} & set(agents)):
        agents = [a for a in ("company", "db") if a not in agents] + agents
        result["mode"] = "agents"
        notes.append(f"입력값 재검토: 대상기업({result['company_id']}) 특정 → 관련 서비스 우선 선정")
    result["agents"] = agents
    if notes:
        result["reasoning"] = (result.get("reasoning", "") + " · " + " · ".join(notes)).strip(" ·")

_AGENT_ORDER = [
    "ocr", "summary", "rag_create", "clearance_report", "company", "db",
    "rag_customs", "rag_trade", "rag_audit", "rag_investigation", "rag_global", "rag_item",
    ...
    "report", "validate", "mail_share", "address_check",
]

_AGENT_LABEL = {
    "company":            "기업 기본정보",
    "db":                 "CDW 자연어조회",
    "db_external":        "전자통관외부정보조회",
    "external_agency":    "외부기관정보수집",
    "rag_customs":        "관세정보 RAG",
    "rag_trade":          "무역정보 RAG",
    "rag_audit":          "심사정보 RAG",
    "rag_investigation":  "조사정보 RAG",
    ...
    "ocr":                "OCR/문서인식",
    "summary":            "보고서 요약",
    "rag_create":         "업무특화RAG 분석서비스",
    "clearance_report":   "통관보고서 생성",
}
...
# 첨부 파일에 대한 요청 의도 → 실행할 전용 AI 서비스.
# 내용 인식은 OCR 시뮬레이션, 요약은 파일 요약, 지식기반 구축은 RAG 생성이 담당한다.
# 위에서부터 먼저 일치하는 항목을 쓰므로 포괄적인 '내용 인식'을 마지막에 둔다.
_FILE_TASK_ROUTES = (
    ("rag", ("rag", "지식기반", "지식 기반", "지식베이스", "지식 베이스", "검색db", "검색 db",
             "컬렉션", "임베딩", "색인", "인덱싱", "지식으로"),
     ["ocr", "rag_create"], "첨부 문서를 지식기반으로 구성 — RAG 생성 서비스 호출"),
    ("summary", ("요약", "간추", "핵심만", "정리해", "summary", "summarize"),
     ["ocr", "summary"], "첨부 문서 요약 — 파일 요약 서비스 호출"),
    ("read", ("인식", "판독", "추출", "읽어", "읽고", "내용 확인", "내용확인", "ocr", "스캔", "분석해"),
     ["ocr"], "첨부 문서 내용 인식 — OCR 서비스 호출"),
)


def _file_task_route(prompt: str) -> tuple[list[str], str]:
    """첨부 파일 요청의 의도를 분류해 (실행 서비스 목록, 판단 사유)를 돌려준다.
       해당 없으면 ([], "") — 이 경우 기존 LLM 라우터 판단을 따른다."""
    text = prompt.lower().replace("프로파일", " ").replace("프로필", " ")
    for _kind, words, agents, reason in _FILE_TASK_ROUTES:
        if any(word in text for word in words):
            return list(agents), reason
    return [], ""
...
def analyze_prompt_intent(body: dict) -> dict:
    ...
    # 첨부 파일 작업은 의도에 맞는 전용 서비스로 라우팅한다
    # (내용 인식 → OCR 시뮬레이션 / 요약 → 파일 요약 / RAG·검색DB 지식기반 → RAG 생성).
    file_task_agents, file_task_reason = _file_task_route(prompt) if attachment_inputs else ([], "")

    if _is_attachment_direct_task(prompt, attachment_inputs):
        # 파일만 다루는 요청이라도 해당 서비스가 있으면 LLM 자체 답변 대신 그 서비스를 실행한다.
        if file_task_agents:
            return {
                "mode": "agents",
                "company_id": None,
                "reasoning": file_task_reason,
                "agents": file_task_agents,
                "llm_answer": "",
                "agent_defs": [
                    {"type": key, "key": key, "label": _AGENT_LABEL.get(key, key)}
                    for key in file_task_agents
                ],
            }
        return {
            "mode": "llm_direct",
    ...
    # ── 라우팅 후처리 규칙 ──────────────────────────────────────────────
    if result["mode"] == "agents":
        agents = [a for a in result["agents"] if a]
        # (1) 보고서 생성 금지 — 검색 결과를 보고서 형태로 만들지 않는다
        dropped_report = [a for a in agents if a in _REPORT_AGENTS]
        agents = [a for a in agents if a not in _REPORT_AGENTS]
        # (2) 대상(기업/개인) 미특정 시 대상특정-필수 에이전트 제외
        dropped_target = []
        if not result["company_id"]:
            dropped_target = [a for a in agents if a in _TARGET_REQUIRED_AGENTS]
            agents = [a for a in agents if a not in _TARGET_REQUIRED_AGENTS]
        result["agents"] = agents
        if dropped_report:
        ...
        # (3) 파일 첨부 시 파일 처리 서비스를 선두 삽입 — 대상특정 서비스가 제외돼도 파일은 읽는다.
        #     요청 의도에 맞는 서비스(OCR/요약/RAG 생성)를 쓰고, 의도가 불분명하면 OCR+요약 기본값.
        if attachment_inputs:
            if any(_looks_like_communication_file(f) for f in attachment_inputs) and "network" not in agents:
                agents = ["network"] + agents
            leading = file_task_agents or ["ocr", "summary"]
            missing = [key for key in leading if key not in agents]
            if missing:
                agents = missing + agents
                if file_task_reason:
                    result["reasoning"] = (result["reasoning"] + " · " + file_task_reason).strip(" ·")
            result["agents"] = agents
    ...
    # 라우터가 llm_direct 로 판단했더라도, 첨부 파일에 대한 처리 요청이면 해당 서비스를 실행한다.
    # (예: "RAG로 등록해줘"·"파일 읽고 신고검증에 써줘" — 내부 업무어가 섞여 직접처리 경로를 타지 못한 경우)
    if attachment_inputs and file_task_agents and result["mode"] != "agents":
        result["mode"] = "agents"
        result["agents"] = list(file_task_agents)
        result["llm_answer"] = ""
        result["reasoning"] = ((result.get("reasoning") or "") + " · " + file_task_reason).strip(" ·")

    # ── 입력값 2차 검토 — LLM 판단을 실제 입력 신호(주소·대상기업)로 재확인·보정 ──
    #    (주소 입력 → 주소확인, 대상기업 특정 → 관련 서비스 우선). llm_direct도 신호가 있으면 agents로 전환.
    _recheck_by_input_signals(prompt, result)
...
_DATA_SOURCES = {
    "db_cdw":          "CDW 자연어조회 — agent_db.py, DuckDB 내 정보만 제공",
    "db_external":     "전자통관외부정보조회 — agent_external_info.py, 국세청 세적·한국은행 수신자료(모의 연계)",
    "external_agency": "외부기관정보수집 — agent_external_info.py, DART·NICE·CRETOP 등 외부 사이트 수집 포인트",
    ...
    "rag_item":        "품목정보 RAG — agent_rag.py의 rag_item 컬렉션 검색(HSK 품목별 신고 가이드 2,583개)",
}
