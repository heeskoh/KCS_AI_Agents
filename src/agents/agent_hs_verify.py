...
분석 항목:
1. 품목분류 적정성: 수입신고 품목명·규격·용도와 신고 HS 코드의 적합성을 검토
2. 세금계산서/첨부문서 비교: 세금계산서 물품목록과 수입신고 물품목록이 품명·모델·수량·단가 측면에서 일치하는지 확인
3. 대체 HS 후보: 신고 HS 코드가 부적정할 경우 가능한 대체 HS와 판단 근거를 제시
4. 전략물자 해당 여부: HS 코드와 품목 설명을 기준으로 전략물자 가능성, 수출허가 필요성, 허가내역 확인 포인트를 도출
5. 원산지·FTA 영향: 원산지 증빙과 FTA 혜택이 품목분류 변경 시 어떻게 달라지는지 설명
6. 필요 서류: 품목분류 확정과 전략물자 검증을 위해 우선 확보할 서류 목록

[품목분류 검증 데이터]
{hs_raw}

관세법 제86조(품목분류 적용기준), FTA 특례법, 관세법 제23조(특수관계)를 인용하여
이는 실제 처분이 아니라 OpenAI 기반 시뮬레이션 결과임을 전제로, 간결하고 실무적으로 작성하세요.
"""
