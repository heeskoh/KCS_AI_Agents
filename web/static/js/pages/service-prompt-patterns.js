...
- 후속 확인 항목`,
  ...
  address_check: {
    ...
    scenarioInputs: ctx => [
      `분석대상 ${ctx.targetLabel || "(조사 대상 미선택)"}`,
      "등록 기초자료의 주소(사업장·주소지 자동 추출)",
    ],
    fallbackDetail: `[수행 절차]
1. 입력 주소를 도로명/지번으로 정규화하고 건축물대장·지도·상권 정보와 대조하십시오.
2. 건물 용도(주거용/근린생활·업무·판매시설 등)와 층별 구성으로 가정집인지 상가건물인지 판별하십시오.
3. 사업장 주소로 신고된 경우 실재성(위장 사업장·유령 주소 가능성)을 함께 검토하십시오.

[출력 형식]
- 판정: 가정집(주거용) / 상가건물(사업용) / 복합 (판정 불가 시 "확인 필요")
- 건물 정보·근거 표 (용도·층수·대조 출처)
- 조사 시사점 (위장 사업장 가능성 등 후속 확인 항목)`,
  },
  mail_share: {
    kind: "agent",
    label: "내부메일 공유",
...
- 종합 의견`,
  ...
  db_external: {
    kind: "kb",
    provides: "전자통관 연계 외부기관 자료(국세청 세적·한국은행 수신) 조회",
    behaviorDesc: {
      "국세청세적자료": "국세청 세적자료(사업자 상태·과세유형·체납)를 조회하고",
      "한국은행수신자료": "한국은행 수신자료(외환 수신 내역·상대국)를 확인하고",
    },
  },
  external_agency: {
    kind: "agent",
    label: "외부기관정보수집",
    homeInputs: ["대상 기업", "수집 기관 선택"],
    scenarioInputs: ctx => [
      `분석대상 ${ctx.targetLabel || "(조사 대상 미선택)"}`,
      "선택한 외부기관 사이트 목록(DART·NICE·CRETOP 등, URL 자동 연계)",
    ],
  },
  db_cdw: {
    kind: "kb",
    provides: "CDW(관세 데이터 웨어하우스) 자연어 조회",
    behaviorDesc: {
      "기업프로파일조회": "기업 프로파일을 조회하고",
      "통합위험정보조회": "통합 위험정보를 확인하고",
      "조사및소송 이력조회": "조사·소송 이력을 확인하고",
      "수출입신고내역조회": "수출입신고 내역을 조회하고",
    },
  },
};
