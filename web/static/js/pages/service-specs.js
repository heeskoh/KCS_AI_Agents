...
export const SERVICE_SPECS = {
  ...
  "주소확인": {
    tag: "외부연계", desc: "주소를 입력하면 카카오지도(카카오 로컬) API로 주소·좌표·건물명과 해당 위치 등록 사업장(상호)을 확인해 개인주소(주거용)인지 사업장(상업용)인지 판별합니다. API 키(KAKAO_REST_API_KEY) 미설정 시 주소 표기 휴리스틱으로 모의 판정합니다.",
    inputs: [
      { name: "확인 주소", type: "텍스트", req: true, source: "직접 입력", rule: "도로명 또는 지번 주소" },
      { name: "분석범위", type: "다중선택", req: false, source: "기본: 건물용도 판별", rule: "건물용도 판별(개인주소/사업장) · 사업장 실재성 확인" },
    ],
    output: { format: "주소 확인 판정 표", fields: [["판정", "개인주소(주거용) / 사업장(상업용) / 복합 / 확인 필요"], ["주소 확인", "도로명·지번·좌표·건물명(카카오지도)"], ["등록 사업장", "해당 위치 카카오 플레이스 상호·카테고리"], ["조사 시사점", "위장 사업장·유령 주소 가능성"]] },
    checks: ["주소 형식(도로명/지번) 유효성 확인", "판정 불가 시 '확인 필요' 표기(단정 금지)", "API 미연계 시 모의 판정임을 명시"],
    sample: ["서울 금천구 가산디지털1로 951 — 사업장(상업용) 판정, 등록 상호와 신고 업체명 대사 권고"],
  },
  "내부메일 공유": {
