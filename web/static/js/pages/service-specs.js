...
export const SERVICE_SPECS = {
  ...
  "통관보고서 생성": {
    tag: "보고서", desc: "수입신고 1건을 대상으로 신고내용을 조회하고 현장 사진을 증빙으로 첨부해 통관보고서를 등록합니다.",
    inputs: [
      { name: "신고번호", type: "식별자", req: true, source: "직접 입력 / CDW 조회 결과에서 선택", rule: "예: DV2-C-1001-01 · 41234-26-100001U" },
      { name: "현장 사진", type: "이미지", req: true, source: "파일첨부", rule: "1장 이상 (png·jpg 등)" },
    ],
    output: { format: "Markdown 보고서", fields: [["신고 내용", "CDW에서 조회한 신고·거래·운송·세액"], ["현장 증빙사진", "사진별 판독 설명"], ["통관 의견", "현품 확인·확인필요·수리/보완요구/검사강화"]] },
    checks: ["신고번호 존재 확인", "증빙사진 1장 이상 등록", "사진에 없는 사실 단정 금지"],
    sample: ["DV2-C-1001-01 · 증빙 1장 · 의견: 보완요구"],
  },
  "무역정보 RAG 분석": {
    tag: "업무분석", desc: "통관/무역 정보의 이상 징후와 참고 근거를 확인합니다.",
    inputs: [
      { name: "대상 기업/개인", type: "식별자", req: true, source: "시나리오·기초자료: 자동 / My AI: 직접 입력", rule: "C-#### 또는 P-#### 형식" },
      { name: "통관/무역 데이터", type: "데이터셋", req: true, source: "CDW·업로드 자료", rule: "기간·대상 지정 필요" },
    ],
    output: { format: "Markdown 요약", fields: [["이상 징후", "탐지 항목 요약"], ["참고 근거", "출처별 인용"], ["권고", "후속 조치 제안"]] },
  ...
  "주소확인": {
    tag: "외부연계", desc: "주소를 번지 단위로 분해해 건축물대장 주용도로 개인주소(가정집)인지 사업장인지 판별합니다. ① 행안부 도로명주소 API로 법정동코드·번지·공동주택 구분을 얻고 ② 국토부 건축HUB 건축물대장 표제부의 주용도코드명으로 판정합니다. 키 미설정 시 카카오지도 로컬 API 또는 주소 표기 휴리스틱으로 폴백합니다.",
    inputs: [
      { name: "확인 주소", type: "텍스트", req: true, source: "직접 입력", rule: "도로명 또는 지번 주소" },
      { name: "분석범위", type: "다중선택", req: false, source: "기본: 건물용도 판별", rule: "건물용도 판별(가정집/사업장) · 사업장 실재성 확인" },
    ],
    output: { format: "주소 확인 판정 표", fields: [["1차 판정", "가정집 / 사업장 / 확인"], ["번지 분해", "도로명·지번·법정동코드·번지(행안부 도로명주소 API)"], ["건축물대장", "주용도코드명·건물명·세대수(건축HUB 표제부)"], ["조사 시사점", "위장 사업장·유령 주소·건축물대장 미등재 가능성"]] },
    checks: ["도로명주소 매칭·번지 분해 성공 확인", "건축물대장 미등재 시 위험 신호 플래그", "혼합용도·미상 시 '확인' 표기(단정 금지)"],
    sample: ["서울 금천구 가산디지털1로 951 — 건축물대장 주용도 '업무시설'로 사업장 판정, 등록 상호와 신고 업체명 대사 권고"],
  },
  "내부메일 공유": {
