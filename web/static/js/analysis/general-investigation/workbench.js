...
export function renderWorkbenchPanel(deps){
  ...
  const selStep = deps.activeGiStep();
    ...
    return `
      <div class="scenario-chip ${chipCls[step.type]||"agent"}${isActive ? " active" : ""}${isDone ? " gi-chip-done" : ""}"
        ...
        <div class="chip-body">
          ...
          <p style="margin:0;font-size:12px;color:#64748b">${escapeHtml(typeLabel[step.type]||step.type)} · ${escapeHtml(deps.giScenarioInstructionPreview(step, aCase.targetType).slice(0,60))}</p>
        </div>
    ...
    <div class="scenario-agent-zone" style="overflow-y:auto;flex:1;min-height:0">
      <label class="scenario-field">
        ...
        <select id="giWbStepSource" class="gi-reg-select" data-gi-step-id="${escapeHtml(selStep.id)}">
          ${deps.giStepSourceOptionsHtml(deps.canonicalGiStepKey(selStep.key))}
        </select>
      </label>
      <div class="scenario-field">
        <span>동작 선택</span>
        <div id="giWbBehaviorOptions" class="scenario-behavior-options" data-gi-step-id="${escapeHtml(selStep.id)}">
          ${deps.behaviorOptionsHtml(selStep.sourceKey || deps.giCommonSourceKey(selStep.key), selStep.behaviors)}
        </div>
      </div>
      <div class="scenario-source-hint">
        <div class="hint-header">
          <strong>${escapeHtml(deps.scenarioSourceByKey(selStep.sourceKey || deps.giCommonSourceKey(selStep.key))?.label || selStep.label)}</strong>
          <span>${escapeHtml(typeLabel[selStep.type] || selStep.type)}</span>
        </div>
        <p>${escapeHtml(deps.sourceDefaultInstruction(selStep.sourceKey || deps.giCommonSourceKey(selStep.key), aCase.targetType) || "이 단계의 추가 지시를 입력하세요.")}</p>
      </div>
      <label class="scenario-field">
        <span>추가 지시</span>
  ...
  const isGiRunning  = !!deps.getGiRunEventSource();
