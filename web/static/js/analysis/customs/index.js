...
import { ciBaseAnalysisRunning } from "./base-analysis.js";
...
export function createCustomsInvestigation(deps){
  ...
  function activeCustomsJob(){
    const activeCompanyId = deps.getActiveCanvasCompanyId?.() || "";
    if(!activeCompanyId) return null;
    return (deps.activeCanvasJobs?.() || [])
      .find(job => job.companyId === activeCompanyId && deps.canvasJobCategory?.(job) === "관세조사 분석") || null;
  }

  function activeCustomsCase(){
    const job = activeCustomsJob();
    // 등록 직후 기초데이터 분석이 완료되기 전에는 미선택과 동일하게 취급 —
    // 기업조사 프로파일 이후 서브탭이 잠기고 완료 시 다시 활성화된다.
    if(!job || ciBaseAnalysisRunning(job)) return null;
    const company = (deps.getScenarioCompanies?.() || []).find(c => c.company_id === job.companyId);
    return company || { company_id: job.companyId };
  }
  ...
  function investigationPage(pageKey = "investigation"){
    ...
    const tabContext = { case: activeCase, baseAnalysisRunning: ciBaseAnalysisRunning(activeCustomsJob()) };
    return `
  ...
  function investigationTabContent(pageKey = "investigation", context = null){
    const tabs = tabsForPage(pageKey);
    const ctx = context || { case: activeCustomsCase(), baseAnalysisRunning: ciBaseAnalysisRunning(activeCustomsJob()) };
