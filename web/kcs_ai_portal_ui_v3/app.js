...
const pages = {
  home: () => `
    ...
    <div class="grid grid-3">
      ...
      <aside class="home-col-aside">
        <section class="card home-col-card">
          <h3>오늘의 AI 추천</h3>
          <div class="notice-list">
            ${notice("고위험 화물 알림","인천항 도착 화물 중 마약 의심 컨테이너 3건 탐지","09:15")}
            ${notice("저가신고 의심 기업","최근 1개월 저가신고 가능성 높은 기업 12개사","08:40")}
            ${notice("보고서 초안","ABC전자 조사보고서 초안 생성 완료","어제")}
          </div>
        </section>
        <section class="card home-col-card">
          <h3>진행 중 작업</h3>
          ${task("ABC전자 조사 사건","AI 분석 | 문서검증 | 보고서 초안")}
          ${task("특송 이상패턴 분석","ML 모델 실행 중")}
        </section>
      </aside>
    </div>

    <div class="quick-bar">
      <span class="quick-bar-label">빠른 분석 바로가기</span>
      <button class="quick-bar-btn" data-page="profile">🏢 기업 위험도 대시보드</button>
      <button class="quick-bar-btn" data-page="classification">🧾 품목분류 추천</button>
      <button class="quick-bar-btn" data-page="lawsearch">⚖ 법령·판례 검색</button>
      <button class="quick-bar-btn" data-page="document">▤ 문서검증센터</button>
      <button class="quick-bar-btn" data-page="dw">◎ DW 분석</button>
      <button class="quick-bar-btn" data-page="report">▧ 보고서 생성</button>
    </div>
    </div>
  `,
...
document.getElementById("shutdownAllBtn").addEventListener("click", () => {
